...
function Panel.new(x, y, w, h, font)
    ...
    self.scrollY           = 0
    self.rowH              = 64  -- tall enough for: name + status + gap + hp bar
...
function Panel:draw(depot)
    ...
    if self:hasSelection() then
        ...
        for id in pairs(self.selected) do
            ...
            if p then
                if sharedROE == nil then sharedROE = p.roe
                elseif sharedROE ~= p.roe then sharedROE = "mixed" end
            end
        end

        local brb = self.buttons.roe
        brb.x = cx; brb.y = cy; brb.w = bw
        self:_drawBtn(brb, "ROE: " .. (ROE_LABELS[sharedROE] or "Mixed"))
        cy = cy + brb.h + 4
    ...
    self.listTopY = cy  -- record for click testing
    ...
    for _, probe in ipairs(depot.probes) do
        ...
        if ry + self.rowH > cy and ry < cy + listH then
            ...
            local hovered = mx2 > x and mx2 < x+w and my2 > ry and my2 < ry+self.rowH
            if hovered and not selected then
                love.graphics.setColor(0.88, 0.82, 0.68, 0.05)
                love.graphics.rectangle("fill", x+2, ry, w-4, self.rowH)
            end

            -- Color stripe
            love.graphics.setColor(col)
            love.graphics.rectangle("fill", x+4, ry+10, 3, self.rowH-20)
            ...
            love.graphics.print(probe.status, cx+8, ry+8+fh+2)

            -- Weapon badge (top right)
            if probe.weapon ~= "none" then
                love.graphics.setColor(0.70, 0.52, 0.32)
                local ww = self.font:getWidth("[" .. probe.weapon .. "]")
                love.graphics.print("[" .. probe.weapon .. "]", x+w-ww-8, ry+8)
            end

            -- HP bar — sits at the bottom of the row with clear gap from text
            local barY = ry + self.rowH - 14
            local barW = w - 28
            love.graphics.setColor(0.22, 0.08, 0.08)
            love.graphics.rectangle("fill", cx+8, barY, barW, 5)
            love.graphics.setColor(col[1]*0.85, col[2]*0.85, col[3]*0.85)
            love.graphics.rectangle("fill", cx+8, barY, barW*(probe.hp/probe.maxHp), 5)
...
function Panel:probeRowAt(my, depot)
    if self.listTopY == 0 then return nil end
